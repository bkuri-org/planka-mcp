...
export class PlankaError extends Error {
  status: number;
  response: PlankaErrorResponse;

  constructor(message: string, status: number, response: PlankaErrorResponse) {
    super(message);
    this.status = status;
...
export function createPlankaError(status: number, response: PlankaErrorResponse): PlankaError {
  switch (status) {
    case 401:
